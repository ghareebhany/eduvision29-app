...
class AvatarUploadNotifier extends StateNotifier<AvatarUploadState> {
  ...
  Future<bool> upload(String filePath) async {
    ...
    return result.fold(
      ...
      (_) {
        // ✅ مهم: إبطال الكاش بعد رفع الصورة حتى تظهر الصورة الجديدة
        // فورًا في الرئيسية وفي الملف الشخصي بدون إعادة تشغيل التطبيق.
        // كان الكود القديم يكتفي بتحديث state فقط.
        _invalidateProfile();
  ...
  void _invalidateProfile() {
    ...
    if (auth is AuthAuthenticated) {
      _ref.invalidate(profileProvider(auth.user.id));
    }
    CacheManager.instance.invalidatePattern('profile_');
  }
