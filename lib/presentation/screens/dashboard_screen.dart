...
class _Avatar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    //
    // استجابة تسجيل الدخول (JWT) لا تحتوي على avatar_url إطلاقًا، ولذلك كان
    // UserModel.fromLoginJson يضع avatarUrl: '' دائمًا ⇒ الصورة لا تظهر أبدًا
    // في الرئيسية حتى لو كانت موجودة في الملف الشخصي.
    //
    // الحل: نقرأ الصورة من profileProvider (المنفذ /app/v1/profile/me) وهو
    // نفس المصدر الذي تستخدمه شاشة "الملف الشخصي"، ونرجع للحروف الأولى فقط
    // في حال عدم وجود صورة أو فشل تحميلها.
    final id = user?.id;

    var url = user?.avatarUrl ?? '';
    if (url.isEmpty && id != null) {
      url = ref.watch(profileProvider(id)).maybeWhen(
            data: (p) => p.avatarUrl,
            orElse: () => '',
          );
    }
  ...
  /// الصورة الرمزية البديلة: الحروف الأولى من الاسم فوق تدرّج لوني
  Widget _initials() => Container(
        decoration: const BoxDecoration(
          ...
          gradient: LinearGradient(
            ...
            colors: [AppPalette.coral, Color(0xFFE05543)],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    final cards = <_FeatureCardData>[
      _FeatureCardData(
        ...
        route: '/hub/quizzes',
        colors: const [Color(0xFF0F9D8C), Color(0xFF0B6F63)], // تركوازي
      ),
      _FeatureCardData(
        ...
        route: '/hub/qna',
        colors: const [Color(0xFF3E63C8), Color(0xFF2A3F86)], // أزرق نيلي
      ),
      _FeatureCardData(
        ...
        colors: const [Color(0xFF2FA36B), Color(0xFF1B6B45)], // زمردي
      ),
      _FeatureCardData(
